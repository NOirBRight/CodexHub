...
from providers_config import (
    ...
    resolve_ollama_cloud_model as _resolve_ollama_cloud_model_config,
)
...
def _resolved_catalog_path(path: Path) -> Path:
    return _catalog_path_reader(path)


def official_prefixes() -> tuple[str, ...]:
    prefixes = _routing_config_reader().get(
        "official_prefixes", _facts().default_official_prefixes
    )
    if not isinstance(prefixes, list):
        return _facts().default_official_prefixes
    values = tuple((str(prefix) for prefix in prefixes if str(prefix)))
    return values or _facts().default_official_prefixes


def official_base_url() -> str:
    if _official_base_url_reader is not None:
        return _official_base_url_reader()
    value = _routing_config_reader().get(
        "official_upstream_base_url", _facts().official_base_url
    )
    return str(value).rstrip("/") if value else _facts().official_base_url


def ollama_cloud_base_url() -> str:
    if _ollama_base_url_reader is not None:
        return _ollama_base_url_reader()
    value = _routing_config_reader().get(
        "ollama_cloud_base_url", _facts().ollama_cloud_base_url
    )
    return str(value).rstrip("/") if value else _facts().ollama_cloud_base_url
...
def generated_catalog_by_slug(path: Path | None = None) -> dict[str, dict[str, Any]]:
    selected_path = path or _facts().generated_catalog_path
    resolved_path = _resolved_catalog_path(selected_path)
    try:
        if resolved_path.exists():
            document = json.loads(_text_reader(resolved_path, "utf-8-sig"))
            if not isinstance(document, Mapping) or not isinstance(
                document.get("models"), list
            ):
                raise ValueError("catalog root must contain a models list")
        catalog_models = _catalog_models_reader(resolved_path)
    ...
    for model in catalog_models:
        ...
        slug = catalog_identity_slug(canonical_model_id(raw_slug_value))
        if slug in models:
            raise ModelIdentityResolutionError(
...
def generated_catalog_slugs(path: Path | None = None) -> set[str]:
    reader = _generated_catalog_by_slug_reader or generated_catalog_by_slug
    return set(reader(path))


def _catalog_by_slug() -> dict[str, dict[str, Any]]:
    if _catalog_by_slug_reader is not None:
        return _catalog_by_slug_reader()
    return generated_catalog_by_slug()


def published_catalog_model(slug: str) -> dict[str, Any] | None:
    resolved_path = _resolved_catalog_path(_facts().generated_catalog_path)
    if resolved_path == _facts().legacy_generated_catalog_path:
        raise ModelIdentityResolutionError(
    ...
    catalog = (
        _catalog_by_slug_reader()
        if _catalog_by_slug_reader is not None
        else generated_catalog_by_slug(resolved_path)
    )
    identity_slug = catalog_identity_slug(slug)
...
def _published_model(slug: str) -> dict[str, Any] | None:
    if _published_model_reader is not None:
        return _published_model_reader(slug)
    return published_catalog_model(slug)


def _generated_official(slug: str, policy: Any) -> str | None:
    if _generated_official_reader is not None:
        return _generated_official_reader(slug, policy)
    return generated_official_upstream_model(slug, policy)


def _official_alias(slug: str, policy: Any) -> str | None:
    if _official_alias_reader is not None:
        return _official_alias_reader(slug, policy)
    return official_alias_upstream_model(slug, policy)


def _official_fast_variant(slug: str, policy: Any) -> str | None:
    if _official_fast_variant_reader is not None:
        return _official_fast_variant_reader(slug, policy)
    return official_fast_variant_upstream_model(slug, policy)


def _ollama_alias(slug: str, policy: Any) -> UpstreamFacts | None:
    if _ollama_alias_reader is not None:
        return _ollama_alias_reader(slug, policy)
    return ollama_cloud_alias_upstream_model(slug, policy)


def is_internal_route_identity(value: Any) -> bool:
    values: list[Any]
    if isinstance(value, Mapping):
        if _internal_model_reader(value):
            return True
        values = [
            value.get(key)
            for key in (
...
def validate_published_model_for_provider(
    ...
    *,
    ...
    if is_internal_route_identity(model):
    ...
    if (
        _model_visibility_reader(model, missing_is_list=True)
        is not CatalogVisibility.LIST
    ):
    ...
    if "supported_in_api" in model and (
        not isinstance(model["supported_in_api"], bool)
    ):
        raise _catalog_failure(
    ...
    if isinstance(metadata, Mapping):
        ...
        if (
            not isinstance(catalog_upstream, str)
            or canonical_model_id(catalog_upstream) != expected_upstream
        ):
            raise _catalog_failure(
...
def resolve_external_model(slug: str) -> dict[str, Any] | None:
    try:
        return _external_model_reader(slug)
    except ModelIdentityResolutionError:
        raise
    except ValueError as exc:
        raise provider_catalog_failure(
...
def resolve_ollama_cloud_model_checked(
    model_id: str, *, require_api_key: bool
) -> tuple[bool, dict[str, Any] | None]:
    try:
        return _ollama_model_reader(model_id, require_api_key=require_api_key)
    except ModelIdentityResolutionError:
        raise
    except ValueError as exc:
        raise provider_catalog_failure(
...
def catalog_output_limit(model_id: str) -> tuple[int | None, bool]:
    ...
    model = _catalog_by_slug().get(catalog_identity_slug(slug))
    cap = _facts().upstream_max_output_token_caps.get(slug)
    ...
    if valid_cap is not None:
        return (
            min(catalog_value, valid_cap) if catalog_value is not None else valid_cap,
            False,
        )
    metadata = model.get("codex_proxy_metadata")
    context_fallback = (
...
def catalog_max_output_tokens(model_id: str) -> int | None:
    return catalog_output_limit(model_id)[0]
...
def policy_denies_model(model_id: Any, policy: Any) -> bool:
    ...
    if not slug:
        return False
    if (
        slug in policy.denied_models
        or deny_match_model_id(slug) in policy.denied_models
    ):
        return True
    lowered = slug.lower()
...
def policy_denies_any_model(model_ids: tuple[Any, ...], policy: Any) -> bool:
    return any(
        (
            model_id is not None and policy_denies_model(model_id, policy)
            for model_id in model_ids
        )
    )


def generated_official_upstream_model(slug: str, policy: Any) -> str | None:
    prefix = _facts().official_alias_prefix
    ...
    model = _published_model(upstream_model)
    if not model:
        return None
    if is_internal_route_identity(model):
        raise _identity_failure(
    ...
    if not isinstance(metadata, Mapping):
        if upstream_model == "gpt-5.5" and _should_include_model_reader(
...
def official_alias_upstream_model(slug: str, policy: Any) -> str | None:
    ...
    if not upstream_model.startswith(official_prefixes()):
        return None
    return _generated_official(slug, policy)
...
def official_fast_variant_upstream_model(slug: str, policy: Any) -> str | None:
    ...
    upstream_model = _facts().official_fast_variant_base_models.get(fast_model)
    if upstream_model is None:
        return None
    upstream_alias = f"{prefix}{upstream_model}"
    if policy_denies_any_model(
    ...
    if not upstream_model.startswith(official_prefixes()):
        return None
    return _generated_official(upstream_model, policy)
...
def ollama_cloud_runtime_upstream(model_id: str, policy: Any) -> UpstreamFacts | None:
    configured, runtime_model = resolve_ollama_cloud_model_checked(
    ...
    if is_internal_route_identity(slug) or is_internal_route_identity(runtime_model):
        raise _identity_failure(
            f"model identity is internal and cannot be routed: {slug}",
            reason="internal_model",
            provider_id="ollama-cloud",
    ...
    policy_alias = runtime_model.get(
        "alias", f"{_facts().ollama_cloud_alias_prefix}{slug}"
    )
    ...
    if (
        configured_id != slug
        and configured_id != f"{_facts().ollama_cloud_alias_prefix}{slug}"
    ):
        raise _catalog_failure(
    ...
    upstream: UpstreamFacts = {
        ...
        **route_capability_metadata(runtime_model),
    }
    if api_key:
...
def _runtime_ollama(model_id: str, policy: Any) -> UpstreamFacts | None:
    if _ollama_runtime_reader is not None:
        return _ollama_runtime_reader(model_id, policy)
    return ollama_cloud_runtime_upstream(model_id, policy)


def ollama_cloud_alias_upstream_model(slug: str, policy: Any) -> UpstreamFacts | None:
    prefix = _facts().ollama_cloud_alias_prefix
    if not slug.startswith(prefix):
        return None
    upstream_model = slug[len(prefix) :]
    ...
    runtime_upstream = _runtime_ollama(slug, policy)
    if runtime_upstream is not None:
        return runtime_upstream
    if not (
        _should_include_model_reader(slug, policy)
        or _should_include_model_reader(upstream_model, policy)
    ):
        raise _identity_failure(
            f"model is not allowed: {slug}",
            reason="unsupported_model",
...
def _official_upstream(slug: str, upstream_model: str) -> UpstreamFacts:
    return {
        "name": "official",
        "provider_id": "openai",
        "model_id": slug,
...
def choose_upstream(model_id: str) -> UpstreamFacts:
    slug = canonical_model_id(str(model_id))
    if not slug:
    ...
    if official_fast is not None:
        upstream = _official_upstream(slug, official_fast)
        upstream["service_tier"] = _facts().official_fast_variant_service_tier
        return upstream
    official_alias = _official_alias(slug, policy)
    if official_alias is not None:
        return _official_upstream(slug, official_alias)
    discovered_official = _generated_official(slug, policy)
    if discovered_official is not None:
        return _official_upstream(slug, discovered_official)
    ollama_alias = _ollama_alias(slug, policy)
    ...
    external_model = resolve_external_model(slug)
    if external_model is not None:
        return _external_upstream(slug, external_model, policy)
    if "/" in slug:
        raise _identity_failure(
    ...
    runtime_ollama = _runtime_ollama(slug, policy)
    if runtime_ollama is not None:
        return runtime_ollama
    if not _should_include_model_reader(slug, policy):
        raise _identity_failure(
            f"model is not allowed: {slug}",
            reason="unsupported_model",
            provider_id="ollama-cloud",
...
def _external_upstream(
    ...
    if not _should_include_external_model_reader(policy_alias, policy):
        raise _identity_failure(
            f"model is not allowed: {slug}",
            reason="unsupported_model",
            provider_id=provider_hint,
    ...
    return {
        ...
        **route_capability_metadata(external_model),
    }
...
def published_official_context_budgets(
    ...
    state_path = catalog_path.parent / _facts().official_refresh_state_filename
    try:
        payload = json.loads(_text_reader(state_path, "utf-8"))
    ...
    *,
    ...
    def guarded_model(model: Any) -> Any:
        ...
        reported = [
            value
            for value in (
                _positive_int(model.get("context_window")),
                _positive_int(model.get("max_context_window")),
            )
...
def catalog_with_vision_proxy_capabilities(catalog: dict[str, Any]) -> dict[str, Any]:
    if not _vision_proxy_enabled_reader():
        return catalog
    models = catalog.get("models")
    if not isinstance(models, list):
        return catalog
...
def canonical_catalog_models(models: list[Any], policy: CatalogPolicy) -> list[Any]:
    known_official_ids = _known_official_ids_reader()
    for model in models:
        if isinstance(model, Mapping):
    ...
    for model in models:
        ...
        candidate["display_name"] = _official_display_name_reader(
        ...
        fresh = (
            candidate
            if not is_legacy_alias or not existing_is_bare
...
def catalog_with_official_fast_variants(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(models, list):
        return catalog
    policy = _policy_reader(_facts().policy_path)
    models = (_canonical_models_reader or canonical_catalog_models)(models, policy)
    ...
    for (
        fast_model,
        upstream_model,
    ) in _facts().official_fast_variant_base_models.items():
        legacy_base_slug = f"{_facts().official_alias_prefix}{upstream_model}"
...
def catalog_input_modalities(
    model_id: str | None, upstream: Mapping[str, Any] | None = None
) -> Any:
    if _input_modalities_reader is not None:
        return _input_modalities_reader(model_id, upstream)
    candidates: list[str] = []
    for value in (model_id, upstream.get("upstream_model") if upstream else None):
        ...
        if slug.startswith(_facts().official_alias_prefix):
            candidates.append(slug[len(_facts().official_alias_prefix) :])
        else:
            candidates.append(f"{_facts().official_alias_prefix}{slug}")
    catalog = _catalog_by_slug()
    for candidate in dict.fromkeys(candidates):
        model = catalog.get(catalog_identity_slug(candidate))
...
def model_supports_image(
    ...
    image_reader = _modalities_reader or modalities_include_image
    if upstream and image_reader(upstream.get("input_modalities")):
        return True
    return image_reader(catalog_input_modalities(model_id, upstream))
...
def _facts() -> CatalogFacts:
    return CatalogFacts()


resolve_ollama_cloud_model = _resolve_ollama_cloud_model_config


def _catalog_path_reader(path: Path) -> Path:
    return existing_generated_catalog_path(path)


def _catalog_models_reader(path: Path) -> list[dict[str, Any]]:
    return load_catalog_models(path)


_text_reader = _read_text


def _policy_reader(path: Path) -> CatalogPolicy:
    return load_policy(path)


def _routing_config_reader() -> Mapping[str, Any]:
    return load_routing_config()


def _external_model_reader(slug: str) -> dict[str, Any] | None:
    return resolve_external_model_alias(slug)


def _ollama_model_reader(
    model_id: str, *, require_api_key: bool = True
) -> tuple[bool, dict[str, Any] | None]:
    return resolve_ollama_cloud_model(model_id, require_api_key=require_api_key)


def _vision_proxy_enabled_reader() -> bool:
    return gateway_settings.gateway_image_proxy_enabled()


def _known_official_ids_reader() -> set[str]:
    return known_official_model_ids()


def _official_display_name_reader(
    slug: str, model: dict[str, Any], policy: CatalogPolicy
) -> str:
    return official_short_display_name(slug, model, policy)


_catalog_by_slug_reader = None
_generated_catalog_by_slug_reader = None
_published_model_reader = None
_generated_official_reader = None
_official_alias_reader = None
_official_fast_variant_reader = None
_ollama_runtime_reader = None
_ollama_alias_reader = None


def _should_include_model_reader(slug: str, policy: CatalogPolicy) -> bool:
    return should_include_model(slug, policy)


def _should_include_external_model_reader(slug: str, policy: CatalogPolicy) -> bool:
    return should_include_external_provider_model(slug, policy)


def _model_visibility_reader(*args: Any, **kwargs: Any) -> CatalogVisibility:
    return model_visibility(*args, **kwargs)


def _internal_model_reader(value: Any) -> bool:
    return is_internal_model(value)
...
_input_modalities_reader = None
_published_budget_reader = None
