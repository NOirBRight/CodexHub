...
class _UnboundStreamInterruptedError(RuntimeError):
    def __init__(self, cause: BaseException | None = None):
        self.cause = (
            cause if isinstance(cause, BaseException) else RuntimeError("unbound")
        )
...
def _diagnostic_error_connection_disposition_value(exc: BaseException) -> str:
    try:
        disposition = getattr(
            exc, "_codexhub_diagnostic_connection_disposition", "unobserved"
        )
    except Exception:
        return "unobserved"
    return disposition if disposition in {"new", "reused"} else "unobserved"
...
class RetryExecutionPlanLike(Protocol):
    ...
    def capacity_elapsed_limit_allows(
        self, started_at: float, delay_seconds: int | float
    ) -> bool: ...


class DiagnosticRecorder(Protocol):
    def observe_upstream_phase(
        self, request_key: str | None, **fields: Any
    ) -> None: ...
...
def _observe(
    callback: Callable[..., None], request_key: str | None, **fields: Any
) -> None:
    recorder = gateway_events.GATEWAY_DIAGNOSTIC_RECORDER
...
def observe_upstream_phase(request_key: str | None, **fields: Any) -> None:
    _observe(
        lambda recorder, key, **values: recorder.observe_upstream_phase(key, **values),
        request_key,
        **fields,
    )


def observe_upstream_attempt(request_key: str | None, **fields: Any) -> None:
    _observe(
        lambda recorder, key, **values: recorder.observe_upstream_attempt(
            key, **values
        ),
        request_key,
        **fields,
...
def observe_upstream_headers(request_key: str | None, **fields: Any) -> None:
    _observe(
        lambda recorder, key, **values: recorder.observe_upstream_headers(
            key, **values
        ),
        request_key,
        **fields,
...
def current_admission() -> Any | None:
...
def resolved_account_id() -> str | None:
    if _account_id is not None:
        return _account_id()
    return _default_account_id()


def resolved_ollama_api_key() -> str | None:
    if _ollama_api_key is not None:
        return _ollama_api_key()
    return os.environ.get("OLLAMA_API_KEY")


def _retry_attempts(**kwargs: Any) -> int:
    fn = _retry_attempts_for_failure_class or _retry_attempts_for_failure_class
    return fn(**kwargs)


def _capacity_allows(started_at: float, delay_seconds: int) -> bool:
    fn = _capacity_elapsed_allows or _capacity_retry_elapsed_limit_allows
    return fn(started_at, delay_seconds)


def _retry_safety(*args: Any, **kwargs: Any) -> str:
    fn = _retry_safety_class
    if fn is None:
...
def _failure_phase(exc: BaseException | None) -> str | None:
    return transport_failure_phase(exc)


def _model_access_path(
    event_context: Mapping[str, Any] | None, upstream_name: str, upstream_format: str
) -> tuple[str, ...]:
    return _model_access_path_from_event_context(
        event_context, upstream_name, upstream_format
    )


def _path_idempotent(path: tuple[str, ...]) -> bool:
    return _model_access_path_idempotency_guaranteed(path)


def _retry_identity(event_context: Mapping[str, Any] | None) -> str | None:
    fn = _retry_identity_from_context
    if fn is None:
        return None
    return fn(event_context)


def _retry_payload(**fields: Any) -> Any:
    fn = _downstream_retry_payload
    if fn is None:
        return fields
    return fn(**fields)


def _observe_context_value(event_context: Mapping[str, Any] | None, key: str) -> Any:
    fn = _diagnostic_context_value
    if fn is None:
...
def _observe_connection_disposition(response: Any) -> str:
    fn = _diagnostic_connection_disposition
    if fn is None:
        return _diagnostic_connection_disposition_value(response)
    return fn(response)


def _observe_error_disposition(exc: BaseException) -> str:
    fn = _diagnostic_error_connection_disposition
    if fn is None:
        return _diagnostic_error_connection_disposition_value(exc)
    return fn(exc)


def _observe_response_metadata(response: Any) -> tuple[Any, Any]:
    fn = _diagnostic_response_metadata
    if fn is None:
        return _response_metadata(response)
    return fn(response)


def _observe_transport_phase(failure_phase: str | None) -> str | None:
    fn = _diagnostic_transport_phase
    if fn is None:
...
def open_once(
    ...
    *,
    ...
    selected_transport = transport_policy or (
        ...
        if upstream_name == _facts().official_upstream_name
...
def build_request(
    ...
    *,
    ...
    if _endpoint_url_hook is None:
        raise RuntimeError("Gateway transport endpoint URL hook is not configured")
    parsed = urlsplit(path)
    endpoint_path = parsed.path or "/"
    url = _endpoint_url_hook(upstream, endpoint_path)
    if parsed.query:
        url += ("&" if "?" in url else "?") + parsed.query
    return build_request_url(url, data=data, headers=headers, method=method)


def proxy_url(url: str) -> str | None:
    if _proxy_url_hook is not None:
        return _proxy_url_hook(url)
    return official_proxy_url(
        url,
        getproxies_fn=_getproxies or getproxies,
        getproxies_registry_fn=_getproxies_registry or getproxies_registry,
        proxy_bypass_fn=_proxy_bypass or proxy_bypass,
        platform=_platform,
    )


def pool_manager(url: str) -> Any:
    if _pool_manager_hook is not None:
        return _pool_manager_hook(url)
    return official_pool_manager(
        url,
        pools=_official_pools,
        pools_lock=_official_pools_lock,
        proxy_url=proxy_url(url),
    )


def materialize_authentication(
    incoming_headers: Mapping[str, str] | Any, upstream: Mapping[str, Any]
) -> OperationalAuthentication:
...
def failure_class(exc: BaseException) -> str:
    return (_failure_class_hook or _upstream_failure_class)(exc)


def retry_after_seconds(exc: BaseException | None) -> int | None:
    return (_retry_after_hook or _retry_after_delay_seconds)(exc)


def open_response(request: Request, **kwargs: Any) -> Any:
...
def official_proxy_url(
    ...
    *,
    ...
    resolved_registry = (
        getproxies_registry
        if getproxies_registry_fn is None
        else getproxies_registry_fn
    )
...
def _subscription_authorization(
    ...
    *,
    ...
    if not authorization:
        raise ValueError(
            f"subscription credential is missing for auth mode: {auth_mode}"
        )
        ...
        if strategy == AuthenticationStrategy.CODEX_AUTH:
            return OperationalAuthentication(
                ...
                generated_session_id=(
                    read_header(incoming_headers, "Session-id") or make_id()
                ),
                generated_client_request_id=(
                    read_header(incoming_headers, "X-client-request-id") or make_id()
                ),
            )
        return OperationalAuthentication(
            strategy,
...
def transport_failure_phase(exc: BaseException | None) -> str | None:
    ...
    if (
        "unexpected_eof" in detail
        or "ssleoferror" in detail
        or "eof occurred in violation" in detail
    ):
        return "tls_handshake"
    ...
    if (
        ...
        or "connectionreseterror" in detail
...
def _failure_class_from_error_values(values: set[str]) -> str | None:
    ...
    if _http_error_values_contain(values, PERMANENT_UPSTREAM_ERROR_NEEDLES):
        return RETRY_FAILURE_PERMANENT
    if any(
        value in PROVIDER_THROTTLE_ERROR_VALUES for value in values
    ...
    if any(
        value in PROVIDER_OVERLOADED_ERROR_VALUES for value in values
    ) or _http_error_values_contain(
        values,
        PROVIDER_OVERLOADED_ERROR_NEEDLES,
    ):
...
def upstream_failure_class(exc: BaseException) -> str:
    ...
    if isinstance(exc, HTTPError):
        ...
        value_class = _failure_class_from_error_values(values)
        if (
            value_class in CAPACITY_RETRY_FAILURE_CLASSES
            and _status_allows_capacity_error_value(status)
        ):
            return value_class
...
def _open_upstream_response(
    ...
    *,
    ...
    opener = _open_once_hook or open_once
    if retry_execution is not None:
        ...
        explicit_max_attempts = retry_execution.open_attempt_budget is not None
        ...
        base_retry_attempts = (
            (_upstream_retry_attempts_override or _upstream_retry_attempts)(
                request_kind
            )
    ...
    if open_attempt_budget is not None:
        remaining_open_attempts = max(
            ...
            - open_attempt_budget["attempts_started"],
        )
    ...
    diagnostic_request_key = _observe_context_value(event_context, "request_id")
    diagnostic_model = _observe_context_value(event_context, "model")
    model_access_path = _model_access_path(
        event_context,
        upstream_name,
        upstream_format,
    )
    ...
    while True:
        ...
        if _path_idempotent(model_access_path):
            _ensure_retry_identity(
        ...
        try:
            ...
            remaining_budget = _remaining_pre_response_budget_seconds(
                pre_response_deadline
            )
            ...
            connection_disposition = _observe_connection_disposition(response)
            ...
            diagnostic_status, diagnostic_headers = _observe_response_metadata(response)
            ...
            if admission is not None:
                admission.raise_if_cancelled()
            elapsed_ms = int(max(0.0, time.monotonic() - attempt_started_at) * 1000)
            connection_disposition = _observe_error_disposition(exc)
            try:
                transport_phase = _failure_phase(exc)
            except Exception:
            ...
            retry_safety_failure_phase = _retry_safety_phase(exc) or "unknown"
            apply_retry_safety = (
                upstream_name != _facts().official_upstream_name
                and request_kind == RETRY_REQUEST_MAIN_GENERATION
                and request.get_method() == "POST"
            )
            telemetry_failure_phase = (
                retry_safety_failure_phase if apply_retry_safety else transport_phase
            )
            diagnostic_phase = _observe_transport_phase(transport_phase)
            ...
            if isinstance(exc, HTTPError) and not retry_http_errors:
                ...
                raise
            failure_classification = failure_class(exc)
            downstream_exposed_now = bool(
                downstream_exposed is not None and downstream_exposed()
            )
            retry_safety_class = _retry_safety(
                exc,
                request=request,
                upstream_name=upstream_name,
            ...
            if retry_safety_class in _facts().suppressed_retry_safety_classes:
                observe_upstream_attempt(
                    diagnostic_request_key,
                    attempt=request_attempt,
            ...
            if (
                ...
                or failure_classification == RETRY_FAILURE_PERMANENT
            ):
                raise
            delay_seconds = (
                ...
                if retry_execution is not None
                else (_retry_delay_seconds_override or gateway_retry_delay_seconds)(
                    request_attempt,
                    failure_class=failure_classification,
                    retry_after_seconds=retry_after_seconds(exc),
                )
            ...
            if failure_classification in CAPACITY_RETRY_FAILURE_CLASSES and not (
                retry_execution.capacity_elapsed_limit_allows(
                    retry_elapsed_seconds,
                    delay_seconds,
                )
                if retry_execution is not None
                else _capacity_allows(
                    retry_started_at,
                    delay_seconds,
                )
            ):
            ...
            if (
                downstream_retry_callback is not None
                and retry_policy != RETRY_CONSERVATIVE_PRE_OUTPUT
            ):
                if not downstream_retry_callback(
                    _retry_payload(
                        ...
                        delay_seconds=delay_seconds,
                        failure_class=failure_classification,
                        redact_identity=_retry_identity(event_context),
                    )
                ):
                    raise _facts().downstream_closed_before_retry_error(
                        "downstream closed before upstream retry"
                    )
            (_sleep or sleep_for_retry_with_gateway_cancellation)(delay_seconds)
            attempt += 1
    ...
    def __init__(
        ...
        *,
        ...
        selected_logger = (
            ...
            else type(self).default_logger_provider
        )
        self._logger = (
            selected_logger() if callable(selected_logger) else selected_logger
        )
        self._cancellation_requested = (
            (lambda: admission.cancelled)
            if admission is not None
    ...
    def start(self) -> None:
        ...
        with self._close_lock:
            ...
            thread = threading.Thread(
                target=self._read_upstream, name=self._thread_name, daemon=True
            )
            self._thread = thread
        thread.start()
...
def _model_access_path_idempotency_guaranteed(
...
def _live_access_token() -> str:
    ...
    return codex_auth.access_token()


def _live_account_id() -> str | None:
    import codex_auth

    return codex_auth.account_id()


def _live_retry_delay(*args: Any, **kwargs: Any) -> int:
    import gateway_settings as settings

    return settings.gateway_retry_delay_seconds(*args, **kwargs)


def _live_upstream_retry_attempts(kind: str) -> int:
    import gateway_settings as settings

    return settings._upstream_retry_attempts(kind)


def _live_endpoint_url(upstream: Mapping[str, Any], path: str) -> str:
    import route_plan

    return route_plan._upstream_endpoint_url(upstream, path)


def _facts() -> TransportFacts:
...
_official_open = None
_standard_open = None
_open_once_hook = None
_sleep = sleep_for_retry_with_gateway_cancellation
_active_request = gateway_admission.active_gateway_request
_access_token = _live_access_token
_account_id = _live_account_id
_ollama_api_key = None
_new_id = lambda: str(uuid.uuid4())
_diagnostic_connection_disposition = None
_diagnostic_error_connection_disposition = None
_diagnostic_response_metadata = None
_diagnostic_transport_phase = None
_emit_retry = _emit_upstream_retry_event
_emit_retry_suppressed = _emit_upstream_retry_suppressed_event
_retry_delay_seconds_override = _live_retry_delay
_failure_class_hook = None
_retry_after_hook = None
_capacity_elapsed_allows = None
_ensure_retry_identity = _ensure_retry_attempt_identity
_downstream_retry_payload = _downstream_retry_payload
_upstream_retry_attempts_override = _live_upstream_retry_attempts
_getproxies = None
_getproxies_registry = None
_proxy_bypass = None
_platform = None
_official_pools = OFFICIAL_HTTP_POOLS
_official_pools_lock = OFFICIAL_HTTP_POOLS_LOCK
_pool_manager_hook = None
_proxy_url_hook = None
_endpoint_url_hook = _live_endpoint_url
...
def bind_route_plan_operational_authentication(
    ...
    *,
    ...
    if any(attempt.request_headers.materialized for attempt in plan.attempts):
        raise ValueError("route attempt authentication was already materialized")
    if any(
        attempt.authentication_strategy != operational_authentication.strategy
        or attempt.request_mutation_policy != primary_attempt.request_mutation_policy
        for attempt in plan.attempts
    ):
        raise ValueError("route attempts do not share one authentication/header policy")
