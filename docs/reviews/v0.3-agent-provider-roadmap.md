...
### Re-established 2026-09-20 (pinned CLI 2.1.278, loopback only)

Issue #74 re-observed the contract instead of citing the retired spike. The
observed endpoint/query variant, credential carrier, discovery
request/response/cache shape, request field set, and two-turn tool lifecycle are
recorded in
[the wire observation note](../research/2026-09-20-claude-code-gateway-wire-observation.md),
with the isolated harness at
`scripts/claude_messages_loopback_harness.py`. The representation seam is
re-established as `src-python/anthropic_messages_prototype.py`, which reuses the
existing Chat→Responses translation instead of adding a third translator and
fails closed on explicitly requested safety controls.

The decision stays **scoped PARTIAL**: no live upstream of any of the three
protocols has been exercised, so no production Messages route, client
configuration, or AgentProvider work is unblocked by this round. Downstream
client support remains a Gateway compatibility problem and stays separate from
the AgentProvider/ACP roadmap below.
