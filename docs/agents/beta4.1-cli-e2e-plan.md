...
| `B42-PYTHON` | Repository-wide Python 3.13+ runtime selection; no ambient 3.11 fallback | `ae48a1e`: `scripts/Resolve-CodexHubPython.ps1`, `scripts/codexhub-python.cmd`, Rust runtime resolver, `src-python/codex_proxy.py`, `src-python/catalog_sync.py`, E2E fixture/child-process launchers, `conftest.py` | Launcher/version/preflight tests, Rust lifecycle fixtures, isolated E2E child-process checks, and clear wrong-interpreter failure | local automated gates passed; packaged/live candidate gate pending |
