...
| `B42-PYTHON` | Repository-wide Python 3.13+ runtime selection; no ambient 3.11 fallback | `ae48a1e`, `2e55d8d7`, `84fada6`: `scripts/Resolve-CodexHubPython.ps1`, `scripts/codexhub-python.cmd`, Rust runtime resolver, `src-python/codex_proxy.py`, `src-python/catalog_sync.py`, E2E fixture/child-process launchers, `conftest.py`, and the explicit agent runtime gate | Launcher/version/preflight tests, Rust lifecycle fixtures, isolated E2E child-process checks, and clear wrong-interpreter failure | local automated gates passed; packaged/live candidate gate pending |
