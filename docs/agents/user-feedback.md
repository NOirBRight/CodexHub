...
3. If the change is not immediately effective, the completion Toast or a persistent reminder must name the exact client or runtime the user must restart. Do not restart that runtime automatically. Do not use vague phrases such as “restart if needed.”
