...
class Level12E2EParserTests(unittest.TestCase):
    def test_case_summary_retryable_keeps_protocol_failures_hard(self):
        runner = load_runner_module()

        self.assertTrue(
            runner.case_summary_retryable(
                {"pass": False, "failure_classification": "provider_stream_flake", "protocol_lock_relevant": False}
            )
        )
        self.assertTrue(
            runner.case_summary_retryable(
                {
                    "pass": False,
                    "failure_classification": "scheduler_or_model_prompt_defect",
                    "protocol_lock_relevant": False,
                }
            )
        )
        self.assertFalse(
            runner.case_summary_retryable(
                {"pass": False, "failure_classification": "adapter_defect", "protocol_lock_relevant": False}
            )
        )
        self.assertFalse(
            runner.case_summary_retryable(
                {
                    "pass": False,
                    "failure_classification": "provider_stream_flake",
                    "protocol_lock_relevant": True,
                }
            )
        )
        self.assertFalse(
            runner.case_summary_retryable(
                {"pass": True, "failure_classification": "none", "protocol_lock_relevant": False}
            )
        )

    def test_finalize_attempt_summary_preserves_retry_audit_trail(self):
        runner = load_runner_module()
        first = {
            "case": "level3-k2_7-chat-r01",
            "pass": False,
            "failure_classification": "scheduler_or_model_prompt_defect",
            "checks": {"branch_nodes_seen": False, "no_router_errors": True},
            "stdout": "first.stdout.jsonl",
            "stderr": "first.stderr.txt",
        }
        second = {
            "case": "level3-k2_7-chat-r01-a02",
            "pass": True,
            "failure_classification": "none",
            "checks": {"branch_nodes_seen": True, "no_router_errors": True},
            "stdout": "second.stdout.jsonl",
            "stderr": "second.stderr.txt",
        }

        summary = runner.finalize_attempt_summary(
            "level3-k2_7-chat-r01",
            second,
            [first, second],
            max_attempts=2,
        )

        self.assertEqual(summary["case"], "level3-k2_7-chat-r01")
        self.assertEqual(summary["attempt_case"], "level3-k2_7-chat-r01-a02")
        self.assertEqual(summary["attempt_count"], 2)
        self.assertEqual(summary["case_attempts"], 2)
        self.assertTrue(summary["retried"])
        self.assertEqual(summary["prior_failure_classifications"], ["scheduler_or_model_prompt_defect"])
        self.assertEqual(summary["attempts"][0]["failed_checks"], ["branch_nodes_seen"])
        self.assertEqual(summary["attempts"][1]["stdout"], "second.stdout.jsonl")

    def test_level2_attempt_artifact_suffix_keeps_original_prompt_case(self):
        runner = load_runner_module()
        output_path = Path("run") / "level2-glm52-responses.artifact-r01-a02.txt"
        expected = runner.expected_level2_artifact_text(
            {"model": "ollama-e2e-responses/glm-5.2", "endpoint": "responses"},
            output_path,
            "SENTINEL:level2-glm52-responses-20260706",
        )

        self.assertIn("case: level2-glm52-responses\n", expected)
        self.assertNotIn("artifact-r01-a02", expected)

    def test_subagent_skill_path_resolver_picks_latest_installed_version(self):
        runner = load_runner_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            old = root / "5.1.4" / "skills" / "subagent-driven-development" / "SKILL.md"
            new = root / "6.1.1" / "skills" / "subagent-driven-development" / "SKILL.md"
            old.parent.mkdir(parents=True)
            new.parent.mkdir(parents=True)
            old.write_text("old", encoding="utf-8")
            new.write_text("new", encoding="utf-8")

            self.assertEqual(runner.resolve_subagent_skill_path(root), new)
