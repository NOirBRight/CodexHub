...
export function ProviderDetailPrototype({ provider, onSave, onDelete, notify, onDirty }: {
    ...
    const [operationResult, setOperationResult] = useState('success');
    ...
    {pt(!official && <details className="v-scenario-details"><summary>{pt("模型与端点测试场景")}</summary><select aria-label={pt("模型测试结果")} value={operationResult} onChange={e => setOperationResult(e.target.value)}><option value="success">{pt("成功")}</option><option value="failure">{pt("认证失败")}</option></select></details>)}
    <div className="v-editor-footer"><span>{pt(dirty ? '有未保存更改' : '设置与模型已同步')}</span><button className="v-primary small" disabled={!dirty || saveBusy} onClick={async () => { setFormError(''); if (!draft.name.trim()) {
        setFormError('请填写名称');
        return;
    } try {
