...
type ResourceMetric = { label: string; value: string; unit?: string; note?: string; percent?: number; period?: 'week' };
const resourceFixtures: Record<string, ResourceMetric[]> = {
  openai: [
    ...
    { label: '每周剩余', period: 'week', value: String(prototypeLimits[1].remaining), unit: '%', percent: prototypeLimits[1].remaining, note: '3 天后重置' },
  ],
  xai: [{ label: '每周剩余', period: 'week', value: String(prototypeXaiLimits[0].remaining), unit: '%', percent: prototypeXaiLimits[0].remaining, note: '4 天后重置' }],
...
export function PrototypeResourceDetails({ providerId }: { providerId: string }) {
  ...
  return <div className="v-resource-details" data-count={metrics.length} data-week-only={metrics.length === 1 && metrics[0].period === 'week'}>
