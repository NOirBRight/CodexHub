...
export default function DesktopPrototype() {
    ...
    const [codexScenario, setCodexScenario] = useState('normal');
    ...
    const panelTitle = activeProvider?.name || panel;
    ...
    <main className={`v-content ${page === '使用统计' ? 'analytics-page' : page === '设置' ? 'settings-page' : ''}`}><div className="v-page-heading"><div><h1>{pt(page === '概览' ? '工作空间' : page === 'Provider' ? 'Provider 管理' : page === '客户端' ? '客户端连接' : page === '设置' ? '偏好设置' : '使用统计')}</h1><p>{pt(page === '概览' ? '服务、模型与资源，一眼掌握。' : page === 'Provider' ? '管理模型来源与账户资源。' : page === '客户端' ? '将模型送到你工作的地方。' : page === '设置' ? '按功能归属整理，每一项设置都有清晰的位置。' : '从每一次请求，了解实际使用。')}</p></div><div>{pt(page === '客户端' ? <span className="v-heading-stat"><b>{pt(connectedCount)}</b>{pt(" / 5 已连接")}</span> : page === 'Provider' ? <button className="v-primary small" onClick={() => setPanel('添加 Provider')}><Plus size={13}/>{pt(" 添加 Provider")}</button> : <span className="v-page-date">{pt("本地工作空间 ")}<span>·</span> {pt(new Date().toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' }))}</span>)}</div></div>
    ...
    {pt(panel && <div className="v-overlay" onClick={closePanel}><section className={`v-dialog ${activeProvider || panel === '添加 Provider' ? 'v-dialog-provider' : ''}`} role="dialog" aria-modal="true" aria-label={panelTitle} onClick={e => e.stopPropagation()}><div className="v-dialog-heading"><img className="v-app-symbol" src={brandIcon} alt={pt("CodexHub 图标")}/><div><h2>{panelTitle}</h2><small>{pt("CodexHub 工作空间")}</small></div><button aria-label={pt("关闭")} onClick={closePanel}><X size={16}/></button></div>
    {pt(panel === '添加 Provider' ? <ProviderAddPrototype existing={Object.values(providerDrafts).filter(p => !removed.includes(p.id))} onAdd={addProvider}/> : panel === '手动接入' ? <><p>{pt("在兼容 OpenAI 的客户端填写以下信息。")}</p><label>Base URL<input readOnly value={`http://127.0.0.1:${settings.proxy_port}/v1`}/></label><label>API Key<input readOnly value={settings.gateway_client_key}/></label><div className="v-dialog-hint">{pt("演示凭据，仅展示填写方式。")}</div></> : activeProvider ? <ProviderDetailPrototype onDirty={setProviderDirty} key={activeProvider.id} provider={activeProvider} onSave={async (next) => applyChange('provider-save', () => setProviderDrafts(d => ({ ...d, [next.id]: next })), 'Provider 与模型已保存 · 请重启 Codex；已连接客户端将在下次启动时读取目录')} onDelete={() => action('provider-delete', () => { setRemoved(ids => [...ids, activeProvider.id]); setProviderDirty(false); setPanel(''); }, 'Provider 已移除 · 请重启已连接客户端')} notify={notify}/> : clientFixtures.some(c => c.name === panel) ? (() => { const c = clientFixtures.find(c => c.name === panel)!; return <ClientDetailPrototype id={c.id} condition={conditions[c.id] || 'normal'} onCondition={condition => setConditions(s => ({ ...s, [c.id]: condition }))} connected={clients[c.id]} onToggle={() => toggleClient(c.id)} pending={busy.includes(c.id)} port={settings.proxy_port} notify={notify}/>; })() : panel === '演示场景' ? <><p>{pt("仅改变下一次演示操作的反馈，不修改真实配置。")}</p><label>{pt("下一次保存或连接操作")}<select value={scenario} onChange={e => setScenario(e.target.value)}><option value="normal">{pt("成功")}</option><option value="failure">{pt("写入失败 · 保留原配置")}</option><option value="partial">{pt("保存成功 · 部分客户端同步失败")}</option><option value="restart">{pt("保存成功 · Codex 重启失败")}</option></select></label><label>{pt("Codex 连接前置状态")}<select value={codexScenario} onChange={e => setCodexScenario(e.target.value)}><option value="normal">{pt("正常")}</option><option value="running">{pt("Codex 正在运行 · 重启确认")}</option><option value="foreign">{pt("其他通道占用 · 接管确认")}</option><option value="unsupported">{pt("当前平台不支持自动重启")}</option></select></label><div className="v-dialog-hint">{pt("客户端异常在各客户端详情预览；授权、更新、诊断状态在对应界面预览。")}</div><button className="v-primary" onClick={() => setPanel('')}>{pt("使用此场景")}</button></> : <><p>{pt("紧凑桌面界面交互原型。Gateway 运行、Codex 接入和外部客户端连接分别管理。")}</p><div className="v-dialog-hint">{pt("统计直接使用项目现有组件，数据为内存生成的演示请求。")}</div></>)}
    </section></div>)}
