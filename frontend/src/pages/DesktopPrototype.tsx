...
import { PrototypeResourceDetails } from './PrototypeResourceDetails';
...
function DesktopPrototypeWorkspace() {
    ...
    const [providerTab, setProviderTab] = useState('模型');
    ...
    function openProvider(id: string, tab = '模型') { setProviderTab(tab); setPanel('provider:' + id); }
    ...
    async function addProvider(p: Provider) { return applyChange('add', () => { setProviderDrafts(d => ({ ...d, [p.id]: p })); setRemoved(ids => ids.filter(id => id !== p.id)); setProviderOrder(ids => ids.includes(p.id) ? ids : [...ids, p.id]); setProviderEnabled(d => ({ ...d, [p.id]: true })); openProvider(p.id); }, 'Provider 已添加 · 可继续授权、发现和编辑模型'); }
    ...
    const providerList = <section className="v-provider-table v-resource-table">
      <div className="v-section-header"><h2>{pt('资源余量')}<span>{displayProviders.filter(p=>p.id!=='openai'||settings.include_official_models).length}{pt(' 个 Provider')}</span></h2><div><span className="v-subtle">{pt(refreshed)}{pt('更新')}</span><button className="v-icon-button" aria-label={pt('刷新额度')} onClick={() => action('refresh', () => setRefreshed(new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})), '演示额度已刷新')}><RefreshCw size={13} className={busy.includes('refresh')?'v-spin':''}/></button><button className="v-small-button" onClick={()=>setPanel('添加 Provider')}><Plus size={12}/>{pt('添加')}</button></div></div>
      <div className="v-resource-columns"><span>Provider</span><span>{pt('资源详情')}</span><span>{pt('启用')}</span></div>
      <div className="v-resource-rows">{displayProviders.filter(p=>p.id!=='openai'||settings.include_official_models).map(p=><div className={`v-resource-row ${providerEnabled[p.id]?'':'muted'}`} key={p.id}>
        <button className="v-provider-identity" onClick={()=>openProvider(p.id)}><Logo src={p.icon} name={p.name}/><span><b>{p.name}</b><small>{pt(providerFixtures.find(f=>f.id===p.id)?.kind || p.kind)}</small></span></button>
        <button className="v-resource-detail-button" aria-label={pt(p.name+' 资源详情')} onClick={()=>openProvider(p.id,p.id==='openai'?'账户与用量':providerDrafts[p.id].auth_capabilities?.includes('subscription:xai_oauth')?'订阅账户':'连接配置')}><PrototypeResourceDetails providerId={p.id}/></button>
        <Switch label={pt(`启用 ${p.name}`)} checked={providerEnabled[p.id]} onClick={()=>{setProviderEnabled(v=>({...v,[p.id]:!v[p.id]}));notify(`${p.name} 已${providerEnabled[p.id]?'停用':'启用'}`);}}/>
      </div>)}</div>
    </section>;
    const providerManagement = <section className="v-provider-management" aria-label={pt('模型来源管理')}>
      <div className="v-management-caption"><span>{pt('模型来源与配置')}</span><span>{pt('管理模型、凭据与上游端点')}</span></div>
      <div className="v-management-scroll">{displayProviders.filter(p => p.name.toLowerCase().includes(query.toLowerCase())).map(p => {
        const draft = providerDrafts[p.id], official = p.id === 'openai';
        const enabledModels = draft.models.filter(m => m.enabled).length;
        return <article className={`v-management-row ${providerEnabled[p.id] ? '' : 'muted'}`} key={p.id}>
          <button className="v-provider-identity" onClick={() => openProvider(p.id)}><Logo src={p.icon} name={p.name}/><span><b>{p.name}</b><small>{pt(official ? 'Codex 官方订阅' : draft.auth_capabilities?.includes('subscription:xai_oauth') ? '订阅 / API Key' : 'API Key')}</small></span></button>
          <div className="v-management-endpoint"><span>{pt(official ? '认证由 Codex 管理' : draft.api_key ? 'API Key 已填写' : '待配置凭据')}<i>·</i>{pt(official ? '官方服务' : ({auto:'自动探测',responses:'Responses',chat_completions:'Chat Completions',anthropic_messages:'Anthropic Messages'}[draft.upstream_format || 'auto']))}</span><code title={draft.base_url}>{draft.base_url}</code></div>
          <div className="v-management-actions"><button className="v-small-button" onClick={() => openProvider(p.id, official ? '账户与用量' : '连接配置')}><Settings2 size={12}/>{pt(official ? '账户' : '配置')}</button><Switch label={pt(`启用 ${p.name}`)} checked={providerEnabled[p.id]} onClick={() => {setProviderEnabled(v=>({...v,[p.id]:!v[p.id]}));notify(`${p.name} 已${providerEnabled[p.id]?'停用':'启用'}`);}}/></div>
          <div className="v-management-models"><button onClick={() => openProvider(p.id)}><Layers size={12}/><b>{enabledModels}<small> / {draft.models.length}</small></b>{pt('模型已启用')}<ArrowRight size={12}/></button><span className="v-model-preview">{draft.models.slice(0,2).map(m=><code key={m.id}>{m.display_name || m.id}</code>)}{draft.models.length>2 && <small>+{draft.models.length-2}</small>}</span><span className="v-management-order">{!official && <><button aria-label={pt(`上移 ${p.name}`)} disabled={providerOrder.indexOf(p.id)<=1} onClick={()=>moveProvider(p.id,-1)}>↑</button><button aria-label={pt(`下移 ${p.name}`)} disabled={providerOrder.indexOf(p.id)===providerOrder.length-1} onClick={()=>moveProvider(p.id,1)}>↓</button></>}</span></div>
        </article>;
      })}{!displayProviders.some(p=>p.name.toLowerCase().includes(query.toLowerCase())) && <div className="v-empty">{pt('没有匹配的 Provider')}</div>}</div>
    </section>;
    ...
    <main className={`v-content ${page === '使用统计' ? 'analytics-page' : page === '设置' ? 'settings-page' : page === '概览' ? 'overview-page' : page === '客户端' ? 'clients-page' : page === 'Provider' ? 'providers-page' : ''}`}><div className="v-page-heading"><div><h1>{pt(page === '概览' ? '工作空间' : page === 'Provider' ? 'Provider 管理' : page === '客户端' ? '客户端连接' : page === '设置' ? '偏好设置' : '使用统计')}</h1><p>{pt(page === '概览' ? '服务、模型与资源，一眼掌握。' : page === 'Provider' ? '配置模型来源，管理可用模型。' : page === '客户端' ? '将模型送到你工作的地方。' : page === '设置' ? '让工作空间按你的习惯运行。' : '从每一次请求，了解实际使用。')}</p></div><div>{pt(page === '客户端' ? <span className="v-heading-stat"><b>{pt(connectedCount)}</b>{pt(" / 5 已连接")}</span> : page === 'Provider' ? <button className="v-primary small" onClick={() => setPanel('添加 Provider')}><Plus size={13}/>{pt(" 添加 Provider")}</button> : <span className="v-page-date">{pt("本地工作空间 ")}<span>·</span> {pt(new Date().toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' }))}</span>)}</div></div>
    {pt(page === '概览' && <>{pt(codexBridge)}<div className="v-overview-metrics">{pt(modelStats)}</div>{pt(providerList)}<div className="v-overview-bottom"><span><Link2 size={12}/>{pt(connectedCount)}{pt(" 个外部客户端已连接")}</span><button onClick={() => setPage('客户端')}>{pt("管理连接 ")}<ArrowRight size={12}/></button><button onClick={() => setPage('使用统计')}>{pt("查看完整统计 ")}<ArrowUpRight size={12}/></button></div></>)}
    {pt(page === '客户端' && <>{pt(codexBridge)}<div className="v-client-section-head"><h2>{pt("外部客户端 ")}<span>5</span><button className="v-small-button" disabled={busy.includes('versions')} onClick={() => action('versions', () => setVersionsChecked(true), '客户端版本已检查 · 无需重启')}><RefreshCw size={12}/>{pt("检查版本")}</button></h2><div className="v-segment">{pt(['全部', '已连接', '未连接'].map(x => <button key={x} className={clientFilter === x ? 'active' : ''} onClick={() => setClientFilter(x)}>{pt(x)}</button>))}</div></div><div className="v-clients-scroll">{pt(clientsGrid)}</div><div className="v-client-note"><ShieldCheck size={13}/><span>{pt("连接时添加 CodexHub 的 Provider 配置；断开时仅移除其管理的配置。")}</span><button className="v-small-button" onClick={() => setPanel('连接参数')}><Copy size={12}/>{pt("连接参数")}</button></div></>)}
    {pt(page === 'Provider' && <><div className="v-provider-tools"><label><Search size={14}/><input aria-label={pt("搜索 Provider")} placeholder={pt("搜索 Provider")} value={query} onChange={e => setQuery(e.target.value)}/></label><span>{pt(activeProviders)}{pt(" 个已启用 · ")}{pt(modelCount)}{pt(" 个可用模型")}</span></div>{pt(providerManagement)}<div className="v-provider-footer"><span>{pt("额度与余额集中显示在概览")}</span><button onClick={() => setPage('概览')}>{pt("查看资源余量")}<ArrowUpRight size={12}/></button></div></>)}
    ...
    <div className={activeProvider?'v-dialog-body v-dialog-body-provider':'v-dialog-body'}>{pt(panel === '添加 Provider' ? <ProviderAddPrototype existing={Object.values(providerDrafts).filter(p => !removed.includes(p.id))} onAdd={addProvider}/> : panel === '连接参数' ? <><p>{pt("适用于支持自定义 Base URL、API Key 和模型 ID 的 OpenAI 兼容客户端。")}</p><label>Base URL<div className="v-key-editor"><input readOnly value={`http://127.0.0.1:${settings.proxy_port}/v1`}/><button aria-label={pt("复制 Base URL")} onClick={() => {void navigator.clipboard.writeText(`http://127.0.0.1:${settings.proxy_port}/v1`).then(()=>notify('端点已复制')).catch(()=>notify('复制失败'));}}><Copy size={13}/></button></div></label><label>API Key<div className="v-key-editor"><input readOnly value={settings.gateway_client_key}/><button aria-label={pt("复制 API Key")} onClick={() => {void navigator.clipboard.writeText(settings.gateway_client_key).then(()=>notify('已复制')).catch(()=>notify('复制失败'));}}><Copy size={13}/></button></div></label><div className="v-dialog-hint">{pt("模型 ID 请从 Provider 的模型列表复制；不同客户端的字段名称和文件格式可能不同。")}</div><div className="v-dialog-hint">{pt("请按目标客户端的配置格式填写。这里仅提供参数，不会自动修改配置、检测连接或加入受管客户端列表。")}</div></> : activeProvider ? <ProviderDetailPrototype onDirty={setProviderDirty} key={activeProvider.id} provider={activeProvider} initialTab={providerTab} onSave={async (next) => applyChange('provider-save', () => setProviderDrafts(d => ({ ...d, [next.id]: next })), 'Provider 与模型已保存 · 请重启 Codex；已连接客户端将在下次启动时读取目录')} onDelete={() => action('provider-delete', () => { setRemoved(ids => [...ids, activeProvider.id]); setProviderDirty(false); setPanel(''); }, 'Provider 已移除 · 请重启已连接客户端')} notify={notify}/> : clientFixtures.some(c => c.name === panel) ? (() => { const c = clientFixtures.find(c => c.name === panel)!; return <ClientDetailPrototype id={c.id} condition={conditions[c.id] || 'normal'} connected={clients[c.id]} onToggle={() => toggleClient(c.id)} pending={busy.includes(c.id)} port={settings.proxy_port} notify={notify}/>; })() : panel === '演示场景' ? <PrototypeTools conditions={conditions} onCondition={(id,value)=>setConditions(s=>({...s,[id]:value}))} onClose={()=>setPanel('')}/> : <><p>{pt("紧凑桌面界面交互原型。Gateway 运行、Codex 接入和外部客户端连接分别管理。")}</p><div className="v-dialog-hint">{pt("统计直接使用项目现有组件，数据为内存生成的演示请求。")}</div></>)}
    </div></section></div>)}
