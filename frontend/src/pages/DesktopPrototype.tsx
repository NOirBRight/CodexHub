...
function DesktopPrototypeWorkspace() {
    ...
    const providerList = <section className="v-provider-table"><div className="v-section-header"><h2>{pt("资源余量 ")}<span>{pt(displayProviders.length)}{pt(" 个 Provider")}</span></h2><div><span className="v-subtle">{pt(refreshed)}{pt("更新")}</span><button className="v-icon-button" aria-label={pt("刷新额度")} onClick={() => action('refresh', () => setRefreshed(new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })), '演示额度已刷新')}><RefreshCw size={13} className={busy.includes('refresh') ? 'v-spin' : ''}/></button><button className="v-small-button" onClick={() => setPanel('添加 Provider')}><Plus size={12}/>{pt(" 添加")}</button></div></div><div className="v-table-labels"><span>Provider</span><span>{pt("剩余额度 / 余额")}</span><span>{pt("资源状态")}</span><span>{pt("启用")}</span></div><div className="v-resource-rows">{pt(displayProviders.filter(p => !removed.includes(p.id) && (p.id !== 'openai' || settings.include_official_models) && providerDrafts[p.id].name.toLowerCase().includes(query.toLowerCase())).map(p => <div className={`v-provider-row ${!providerEnabled[p.id] ? 'muted' : ''}`} key={p.id}><button className="v-provider-identity" onClick={() => openProvider(p.id)}><Logo src={p.icon} name={p.name}/><span><b>{providerDrafts[p.id].name}</b><small>{pt(p.kind)} <i>·</i> {pt(providerDrafts[p.id].models.length)}{pt(" 个模型")}</small></span></button><button className="v-balance" onClick={() => openProvider(p.id)}><strong>{pt(p.unit !== '%' && <em>{pt(p.unit)}</em>)}{pt(p.value)}{pt(p.unit === '%' && <em>%</em>)}</strong><small>{pt(p.caption)}</small></button><div className={`v-quota ${p.id === 'kimi' && page !== '概览' ? 'warning' : ''}`}>{pt(p.percent ? <div className="v-track"><i style={{ width: p.percent + '%' }}/></div> : <span className="v-account-ok">{pt(p.value === '—' ? '未知余量' : <><Check size={11}/>{pt(" 余额正常")}</>)}</span>)}<small>{pt(p.id === 'kimi' && page === '概览' ? '10M Token 总额度' : p.note)}</small></div><div className="v-provider-order">{pt(page === 'Provider' && p.id !== 'openai' && <span><button aria-label={pt(`上移 ${p.name}`)} disabled={providerOrder.indexOf(p.id) <= 1} onClick={() => moveProvider(p.id, -1)}>↑</button><button aria-label={pt(`下移 ${p.name}`)} disabled={providerOrder.indexOf(p.id) === providerOrder.length - 1} onClick={() => moveProvider(p.id, 1)}>↓</button></span>)}<Switch label={pt(`启用 ${p.name}`)} checked={providerEnabled[p.id]} onClick={() => { setProviderEnabled(s => ({ ...s, [p.id]: !s[p.id] })); notify(`${p.name} 已${providerEnabled[p.id] ? '停用' : '启用'}`); }}/></div></div>))}</div></section>;
