...
| GatewayPage：通用接入与配置说明 | 连接参数、客户端详情 | 已保存端口与本地密钥，配置预览、复制，说明受管范围 |
...
标题栏「原型工具」集中管理服务与连接、账户与模型、设置与维护三组验收场景。原各详情中的测试结果下拉框已移出产品页面；成功、失败、部分同步、授权、历史、诊断和更新场景均保留。场景状态在当前演示会话内跨页面保留，可恢复默认值。
...
### Revision 8 navigation refinement

资源余量仅在概览显示；Provider 管理展示模型、认证方式、上游端点与配置入口，完整详情和排序功能保留。客户端页面底部「连接参数」取代「其他客户端」卡片，仅提供手动填写参数与模型 ID 获取说明，不承诺自动适配未知客户端。概览资源列表、Provider 配置列表、客户端卡片区域分别独立滚动，顶部工作空间信息固定。

### Revision 9 resource presentation

概览统一为 Provider / 资源详情 / 启用三列。资源详情按数据数量显示单组或双组：OpenAI 双窗口、xAI 周窗口、余额与今日消耗、Token 剩余/总量。不强制第二指标，不重复显示健康状态；模型数量保留在 Provider 管理。新增 xAI 默认演示账户并同步单周额度详情。
