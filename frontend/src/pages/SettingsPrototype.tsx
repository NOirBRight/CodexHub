...
export function SettingsPrototype({ value, onSave, dark, onTheme, notify, running, providers }: {
    ...
    <div hidden={tab !== 'Codex 与客户端'}><><Section title={pt("模型与连接")} caption={pt("影响 Codex 模型列表以及已接入的客户端。")}><Toggle name="include_official_models" label={pt("包含 OpenAI 官方模型")} description={pt("在共享模型目录中包含官方订阅模型。")}/><Toggle name="auto_sync_clients" label={pt("自动同步已绑定客户端")} description={pt("模型目录更新后，同步已连接客户端的受管配置。")}/><Toggle name="openai_context_guard_enabled" label={pt("OpenAI 上下文保护")} description={pt("从原 OpenAI 模型页迁入；调整官方模型的上下文策略。")}/><ContextGuardStatusPrototype/></Section><Section title={pt("历史对话")} caption={pt("保留原设置中的开关和手动同步入口。")}><Toggle name="unified_codex_history" label={pt("历史对话同步")} description={pt("在支持的 Codex 配置之间统一显示历史对话。")}/><HistoryPrototype notify={notify}/></Section></></div>
