...
export function ContextGuardStatusPrototype(){
  const [status]=usePrototypeScenario('contextGuard');
  return <div className="v-context-status">{status==='conflict'&&<p className="v-dialog-hint" role="status">{pt('全局 model_context_window 覆盖可能阻止逐模型上下文设置生效。请检查 Codex 配置。')}</p>}{status==='unknown'&&<p className="v-dialog-hint" role="status">{pt('上下文保护状态读取失败。请刷新后重试。')}</p>}</div>;
}
