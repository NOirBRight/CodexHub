...
def _request_fields() -> list[dict[str, Any]]:
    ...
    return [
        ...
        _row(
            row_id="request.reasoning.effort",
            category="request_field",
            key="reasoning",
            execution_owner="gateway",
            responses="{effort: none|minimal|low|medium|high|xhigh|max, summary?: auto|concise|detailed}",
            chat="absent",
            disposition="consumed_locally",
            when="exact bounded Codex reasoning-control object",
            fail_closed=False,
            implementation=current,
            notes="Chat cannot return a Responses reasoning summary. The Gateway consumes known effort/summary selectors locally for compatibility; unknown keys or values fail closed.",
        ),
        _row(
        ...
        _row(
            ...
            fail_closed=True,
            implementation=current,
            notes="Known Codex defaults are consumed by value; all other transport semantics fail closed.",
        ),
        _row(
