...
def _request_fields() -> list[dict[str, Any]]:
    ...
    return [
        ...
        _row(
            ...
            key="reasoning",
            execution_owner="gateway",
            responses="{effort: none|minimal|low|medium|high|xhigh}",
            chat="absent",
            disposition="consumed_locally",
            when="exact Codex reasoning-effort object",
            fail_closed=False,
            implementation=current,
            notes="The selected third-party model owns reasoning; unknown keys or values fail closed.",
        ),
        _row(
