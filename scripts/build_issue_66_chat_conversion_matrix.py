...
def _request_fields() -> list[dict[str, Any]]:
    ...
    return [
        ...
        _row(
            ...
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="Chat↔official expands a complete Collaboration V2 six-pack (or aliases) into the native namespace. Third-party Chat still uses injective function aliases.",
        ),
        _row(
            ...
            chat="function alias with exact input/output envelopes, or type=custom on Chat↔official",
            disposition="reversibly_adapted",
            when="exact text-format declaration, or Chat↔official apply_patch alias/type=custom",
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="Chat↔official restores known custom aliases (apply_patch) to native custom. Unknown custom aliases fail closed. Third-party Chat uses the function envelope adapter.",
        ),
        _row(
            row_id="request.tools.custom.unavailable",
            category="tool_declaration",
            key="tools[type=custom|freeform] without an approved adapter",
            execution_owner="codex_client",
            responses="custom/freeform tool",
            chat="none",
            disposition="unavailable",
            when="adapter capability absent, unknown custom alias, or declaration/envelope is malformed",
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="Required custom semantics reject before sampling; optional declarations may be omitted.",
        ),
        _row(
            row_id="request.tools.tool_search",
            category="tool_declaration",
            key="tools[type=tool_search]",
            execution_owner="codex_client",
            responses="client tool_search",
            chat="function alias or type=tool_search execution=client",
            disposition="reversibly_adapted",
            when="Chat↔official client-owned search, or third-party Chat with accepts_tool_search_adapter",
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="A user function merely named tool_search is not client-owned. Hosted search (execution!=client) stays unavailable.",
        ),
        _row(
            row_id="request.tools.hosted",
            category="tool_declaration",
            key="tools[hosted web_search|web_search_preview|file_search|code_interpreter]",
            execution_owner="upstream_provider",
            responses="hosted tool",
            chat="function alias __codexhub_hosted_* or native hosted type",
            disposition="reversibly_adapted",
            when="Chat↔official known hosted kinds with a complete lifecycle",
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="A user function named web_search is not hosted. Official restores alias/type declarations to native hosted tools and collapses hosted calls back to Chat function_call.",
        ),
        _row(
            row_id="request.tools.hosted.unavailable",
            category="tool_declaration",
            key="tools[hosted unknown|incomplete lifecycle]",
            execution_owner="upstream_provider",
            responses="hosted tool",
            chat="none",
            disposition="unavailable",
            when="unknown hosted kind, incomplete lifecycle, encrypted fields, or third-party Chat without provider hosted facts",
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="computer_use_preview and local_shell have no bounded lifecycle. Gateway never synthesizes undeclared Desktop hosted tools.",
        ),
        _row(
        ...
        _row(
            row_id="request.reasoning.controls",
            category="request_field",
            key="reasoning",
            execution_owner="gateway",
            responses="{effort?: none|minimal|low|medium|high|xhigh|max, summary?: auto|concise|detailed, generate_summary?: auto|concise|detailed, mode?: standard|pro, context?: auto|current_turn|all_turns}",
            chat="absent",
            disposition="consumed_locally",
            when="exact bounded Codex reasoning-control object",
            fail_closed=False,
            implementation=current,
            notes="Chat cannot return Responses reasoning controls or summaries. The Gateway consumes documented effort, summary, generate_summary, mode, and context selectors locally for stale Desktop compatibility; unknown keys or values fail closed.",
        ),
        _row(
            row_id="request.client_metadata",
            category="request_field",
            key="client_metadata",
            execution_owner="gateway",
            responses="object",
            chat="absent",
            disposition="consumed_locally",
            when="missing, null, or object",
            fail_closed=False,
            implementation=current,
            notes="Gateway classifies Codex lifecycle metadata locally and never leaks it upstream.",
        ),
        _row(
            ...
            fail_closed=False,
            implementation=current,
            notes="Empty include is a no-op.",
        ),
        _row(
            row_id="request.include.reasoning_encrypted_content",
            category="request_field",
            key="include",
            execution_owner="gateway",
            ...
            chat="absent",
            disposition="consumed_locally",
            when="only reasoning.encrypted_content is requested",
            fail_closed=False,
            implementation=current,
            notes="Third-party Chat cannot provide Official encrypted reasoning; unknown include values fail closed.",
        ),
        _row(
            row_id="request.prompt_cache_key",
            category="request_field",
            ...
            fail_closed=False,
            implementation=current,
            notes="Chat inbound treats prompt_cache_key as a first-class field. Cross-protocol conversion records a cache_control_dropped event for supplied keys on unverified endpoints. Cache usage reporting does not imply key support.",
        ),
        _row(
            ...
            notes="Chat inbound prompt_cache_key is a first-class whitelist field. Preserved in both conversion directions. Same-protocol requests retain original controls and bytes. Invalid key types fail before forwarding.",
        ),
        _row(
        ...
        _row(
            ...
            chat="absent",
            disposition="consumed_locally",
            when="missing, null, empty, or exact Codex verbosity object",
            fail_closed=False,
            implementation=current,
            notes="Unknown structured text keys and verbosity values fail closed.",
        ),
        _row(
            row_id="request.transport.invalid",
            category="request_field",
            key="client_metadata|include|prompt_cache_key|reasoning|text",
            execution_owner="caller",
            responses="wrong type, unknown key, or unknown value",
            chat="none",
            disposition="unavailable",
            when="outside the bounded locally-consumed shapes above",
            fail_closed=True,
            implementation=current,
            notes="Known Codex defaults and documented reasoning selectors are consumed by value; all other transport semantics fail closed.",
        ),
        _row(
...
def _collaboration_v2() -> list[dict[str, Any]]:
    ...
    rows.extend(
        [
            ...
            chat="plaintext envelope __codexhub_agent_message_v2__:",
            disposition="reversibly_adapted",
            when="Chat↔official plaintext author/recipient history",
            fail_closed=True,
            implementation="runtime_tool_compatibility",
            notes="Encrypted agent_message stays unavailable. Third-party Chat still has no native agent_message item.",
        ),
            _row(
