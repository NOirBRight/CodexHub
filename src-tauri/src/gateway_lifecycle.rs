use crate::gateway_transaction::{
    GatewayLifecyclePhase, LifecycleStatusAccess, LifecycleTransactionGate,
};
use crate::AppStatus;
use std::path::Path;
...
impl GatewayLifecycleCoordinator {
    ...
    {
        let _read_guard =
            match LifecycleTransactionGate::inspect_for_status(backend.lifecycle_gate_path())? {
                LifecycleStatusAccess::ReadLocked(guard) => guard,
                LifecycleStatusAccess::Held(phase) => {
...
mod tests {
    ...
    #[test]
    fn status_holds_a_shared_lifecycle_gate_during_snapshot() {
        let coordinator = GatewayLifecycleCoordinator::new();
        ...
        assert!(backend
            .events()
            .iter()
            .any(|event| event == "snapshot-lock-held"));
