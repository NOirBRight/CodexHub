...
impl ManagedClientAdapter for GrokAdapter {
    fn metadata(&self) -> ClientMetadata {
        ClientMetadata {
            id: "grok",
            supports_native: true,
...
pub fn apply_native_at(
    ...
    match spec {
        ...
        NativeApplySpec::Grok { path, backup_roots } => {
            let plan = plan_grok_apply(path, settings, providers, model)?;
            let result = publish_grok_apply(&plan, backup_roots)?;
            if result.applied {
                readback_native_at("grok", &[path.to_path_buf()], settings, providers, model)?;
            }
            Ok(result)
        }
    }
...
pub fn apply_native(
    ...
    match client_id.as_str() {
        ...
        "grok" => {
            ...
            let backup_roots = super::client_backup_roots_for_apply("grok");
            apply_native_at(
                NativeApplySpec::Grok {
                    path: &path,
                    backup_roots: &backup_roots,
                },
                settings,
...
pub fn preview_native(
    ...
    match client_id {
        ...
        "grok" => {
            let path = detect_grok_config_path();
            preview_native_at(
                NativePreviewSpec::Grok { path: &path },
                settings,
                providers,
                model,
            )
...
pub fn apply_native_isolated(
    ...
    match client_id {
        ...
        "grok" => {
            ...
            if let Some(parent) = path.parent() {
                fs::create_dir_all(parent)
                    .map_err(|error| format!("failed to create grok dir: {error}"))?;
            }
            if !path.exists() {
                fs::write(path, "")
                    .map_err(|error| format!("failed to seed grok config: {error}"))?;
            }
            apply_native_at(
                NativeApplySpec::Grok { path, backup_roots },
                settings,
...
mod tests {
    ...
    fn adapter_for_registers_native_clients_and_rejects_unknown() {
        ...
        assert_eq!(
            adapter_for("grok").map(|adapter| adapter.metadata().id),
            Some("grok")
        );
        assert_eq!(
