...
def resolve_subagent_skill_path(cache_root: Path | None = None) -> Path:
    root = cache_root or (
        Path.home()
        / ".codex"
        / "plugins"
    ...
    candidates = list(root.glob("*/skills/subagent-driven-development/SKILL.md"))
    if candidates:
        return max(candidates, key=lambda path: _version_key(path.parents[2].name))
    return root / "6.1.1" / "skills" / "subagent-driven-development" / "SKILL.md"


SKILL_PATH = resolve_subagent_skill_path()
...
def case_summary_retryable(summary: dict[str, Any]) -> bool:
    if summary.get("pass"):
        return False
    if summary.get("protocol_lock_relevant"):
        return False
    return str(summary.get("failure_classification") or "") in RETRYABLE_CASE_FAILURES


def compact_attempt_summary(summary: dict[str, Any]) -> dict[str, Any]:
    checks = summary.get("checks") if isinstance(summary.get("checks"), dict) else {}
    failed_checks = [key for key, value in checks.items() if not value]
    return {
        "case": summary.get("case"),
        "attempt_case": summary.get("attempt_case") or summary.get("case"),
...
def finalize_attempt_summary(
    ...
    *,
    ...
    summary["attempt_case"] = final_summary.get("attempt_case") or final_summary.get("case")
    summary["case"] = original_case_name
    summary["attempt_count"] = len(attempts)
    summary["case_attempts"] = max(1, max_attempts)
    summary["retried"] = len(attempts) > 1
    summary["prior_failure_classifications"] = [
        str(item.get("failure_classification") or "unknown")
        for item in attempts[:-1]
        if not item.get("pass")
    ]
    summary["attempts"] = [compact_attempt_summary(item) for item in attempts]
    return summary
...
def expected_level2_artifact_text(case: dict[str, Any], output_path: Path, sentinel: str) -> str:
    ...
    case_name = re.sub(r"\.artifact(?:-r\d+)?(?:-a\d+)?$", "", artifact_stem)
    return (
        f"case: {case_name}\n"
...
def write_markdown_summary(run_dir: Path, summaries: list[dict[str, Any]], gateway_source: Path) -> None:
    lines = [
        ...
        "",
        "| Level | Case | Mode | Pass | Attempts | Repair | Retry | Resample | Stream | EmptyFinal | Tool counts | Agent ids / roles | Reason |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for item in summaries:
        ...
        case_attempts = int(item.get("case_attempts") or attempt_count)
        attempt_text = f"{attempt_count}/{case_attempts}"
        prior_failures = item.get("prior_failure_classifications") or []
        if prior_failures:
            attempt_text += f" ({','.join(str(value) for value in prior_failures)})"
        ...
        lines.append(
            f"| {level} | `{name}` | {mode} | {passed} | {attempt_text} | repair={repair_count} | retry={retry_count} | resample={resample_count} | stream={stream_count} | empty={empty_final_count} | {count_text} | {ids_or_roles} | {reason} |"
        )
    lines.append("")
...
def task_for_case_attempt(task: dict[str, Any], attempt_index: int) -> dict[str, Any]:
    if attempt_index <= 1:
        return dict(task)
    copy = dict(task)
    base_case_name = str(task["case_name"])
    copy["case_name"] = f"{base_case_name}-a{attempt_index:02d}"
    if "output_path" in copy:
        output_path = Path(copy["output_path"])
        attempt_output_path = output_path.with_name(f"{output_path.stem}-a{attempt_index:02d}{output_path.suffix}")
        copy["output_path"] = attempt_output_path
        if isinstance(copy.get("prompt"), str):
            copy["prompt"] = copy["prompt"].replace(str(output_path), str(attempt_output_path))
    return copy


def analyze_task_case(case: dict[str, Any], task: dict[str, Any]) -> dict[str, Any]:
    if task["scenario"] == "level2":
        return analyze_level2(case, task["output_path"], task["sentinel"])
    if task["scenario"] == "level3_dynamic_dag":
        return analyze_level3_dynamic_dag(case)
    return analyze_level1(case, task["scenario"], task["sentinels"])
...
def run_e2e_task(
    ...
    max_attempts = max(1, int(case_attempts))
    ...
    append_progress(
        ...
        case_attempts=max_attempts,
    )
    for attempt_index in range(1, max_attempts + 1):
        attempt_task = task_for_case_attempt(task, attempt_index)
        append_progress(
            run_dir,
            case=task["case_name"],
            attempt_case=attempt_task["case_name"],
        ...
        case = run_codex_case(
            run_dir=run_dir,
            case_name=attempt_task["case_name"],
            prompt=attempt_task["prompt"],
            model_id=attempt_task["model_id"],
            port=port,
            timeout=attempt_task["timeout"],
            ephemeral_cli=ephemeral_cli,
            preserve_cli_tools=attempt_task["preserve_cli_tools"],
            subagent_mode=attempt_task["subagent_mode"],
            main_retry_attempts=attempt_task["main_retry_attempts"],
        )
        case["endpoint"] = attempt_task["endpoint"]
        case["repeat_index"] = attempt_task.get("repeat_index")
        case["repeat_count"] = attempt_task.get("repeat_count")
        summary = analyze_task_case(case, attempt_task)
        summary["attempt_case"] = attempt_task["case_name"]
        summary["attempt_index"] = attempt_index
        summary["case_attempts"] = max_attempts
        ...
        if summary["pass"]:
            break
        if attempt_index >= max_attempts or not case_summary_retryable(summary):
            break
        append_progress(
            run_dir,
            case=task["case_name"],
    ...
    if final_summary is None:
        raise RuntimeError(f"no E2E attempt ran for {task['case_name']}")
    final = finalize_attempt_summary(task["case_name"], final_summary, attempt_summaries, max_attempts=max_attempts)
    append_progress(run_dir, case=task["case_name"], status="finished", passed=final["pass"])
    return final
...
def run_e2e_tasks(
    ...
    if jobs <= 1 or len(tasks) == 1:
        return [run_e2e_task(run_dir, port, task, ephemeral_cli, case_attempts) for task in tasks]

    summaries_by_index: dict[int, dict[str, Any]] = {}
    with ThreadPoolExecutor(max_workers=max(1, jobs)) as executor:
        futures = {
            executor.submit(run_e2e_task, run_dir, port, task, ephemeral_cli, case_attempts): index
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Retry each E2E case up to this many times when the analyzer classifies the failure as retryable.",
    )
    parser.add_argument(
        "--main-retry-attempts",
        type=int,
    ...
    try:
        ...
        if args.level in {"level1", "all"}:
            ...
            summaries.extend(run_e2e_tasks(run_dir, port, level1_tasks, args.jobs, args.ephemeral_cli, args.case_attempts))
        ...
        if args.level == "level2" or (args.level == "all" and level1_ok):
            ...
            summaries.extend(run_e2e_tasks(run_dir, port, level2_tasks, args.jobs, args.ephemeral_cli, args.case_attempts))
        elif args.level == "all":
        ...
        if args.level == "level3" or (args.level == "all" and level1_ok and level2_ok):
            ...
            summaries.extend(run_e2e_tasks(run_dir, port, level3_tasks, args.jobs, args.ephemeral_cli, args.case_attempts))
        elif args.level == "all":
